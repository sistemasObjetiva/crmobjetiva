...
const IndexPage: React.FC = () => {
  ...
  // --- GRÁFICAS (con seguimientos filtrados si es usuario) ---
  ...
  const seguimientosPorDia = useMemo(() => getSeguimientosPorDia(seguimientosFiltrados), [seguimientosFiltrados])
  ...
  return (
    ...
    >
      <Box mb={2} display="flex" alignItems="center" gap={2}>
        ...
        <Box>
          <Typography variant="h4" fontWeight={900} color="primary" sx={{ letterSpacing: 1, mb: 0.3 }}>
      ...
      {/* Carrusel de proyectos y propiedades */}
      {cards.length > 0 && (
        <Box sx={{ mt: 3, mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 270 }}>
          <Box>
            <Card
              sx={{
                width: 420,
            ...
            >
              ...
              <CardContent sx={{ width: '100%', textAlign: 'center' }}>
                <Typography
                  variant="h6"
                  color={cards[cardIdx].tipo === 'proyecto' ? 'primary' : 'secondary'}
                  fontWeight={700}
                  mb={1}
                >
      ...
      <Paper elevation={1} sx={{ p: 2, mb: 2 }}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <PieChart>
            <Pie data={seguimientosPieData} dataKey="value" innerRadius={50} outerRadius={85} label={({ name, percent }) => `${name} (${Math.round(percent * 100)}%)`}>
      ...
      {/* CALENDARIO reusable (resuelve IDs a nombres y si no encuentra, deja el texto) */}
      <Paper elevation={1} sx={{ p: 2, mb: 2 }}>
