...
const CardPropiedad: React.FC<CardPropiedadProps> = ({
  propiedad,
  onEdit,
  onDelete,
}) => {
  // Elegimos imagen principal (la primera)
  const mainImage = propiedad.imagenes?.[0] ?? null;

  // Dirección completa
  const direccion = [propiedad.calle, propiedad.numero, propiedad.interior ? `Int. ${propiedad.interior}` : '', propiedad.colonia, propiedad.ciudad, propiedad.estado]
    .filter(Boolean)
    .join(', ');
    ...
    >
      ...
      <CardActions sx={{ justifyContent: 'flex-end' }}>
        <Tooltip title="Editar">
          <IconButton color="primary" onClick={() => onEdit(propiedad)}>
            <EditIcon />
          </IconButton>
        </Tooltip>
        <Tooltip title="Eliminar">
          <IconButton color="error" onClick={() => onDelete(propiedad)}>
            <DeleteIcon />
          </IconButton>
        </Tooltip>
      </CardActions>
