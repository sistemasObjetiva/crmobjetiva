...
const ProspectosGeneralTab: React.FC<ProspectosTabProps> = ({ }) => {
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      // 3) Mapas de apoyo (usuarios por email, prospectos existentes por correo)
      ...
      const existingByEmail = new Map<string, string>() // email → prospecto.id
      ;(prospectos ?? []).forEach(p => {
        const em = normEmail(p.correoElectronico)
        if (em && p.id) existingByEmail.set(em, p.id)
      })
      ...
      for (const r of normRows) {
        const nombre = (r['nombre_completo_cliente'] ?? r['nombre_completo'] ?? r['nombre'] ?? '').toString().trim()
        if (!nombre) continue

        const email  = normEmail(r['correo_electronico_cliente'] ?? r['correo'])
        const celular = normPhone(r['celular_cliente'] ?? r['celular'])

        // evita duplicados dentro del archivo por email+celular
        const dk = `${email ?? ''}|${celular ?? ''}`
        if (seen.has(dk)) continue
        seen.add(dk)

        // fecha y campos varios
        ...
        // intereses: "proyecto" (una) y/o "proyectos_interes" (varias)
        ...
        // dueño (userid): intenta por columna "vendedor" (correo); si no, respaldo
        const vendEmail = normEmail(r['vendedor'])
        const ownerId = (vendEmail && usersByEmail.get(vendEmail)) || fallbackUserId

        // id: si existe por correo → actualiza; si no, nuevo UUID
        const id = (email && existingByEmail.get(email)) || crypto.randomUUID()
