...
export const handleDownloadPDF = async (
  ...
  try {
    ...
    if (selectedPlan) {
      ...
      y += 6;
      pdf.text(`Pago Inicial: ${formatoMoneda((+unidad.preciolista * selectedPlan.pInicial) / 100)}`, 15, y);
      y += 6;
      pdf.text(`Pago Contraentrega: ${formatoMoneda((+unidad.preciolista * selectedPlan.contraentrega) / 100)}`, 15, y);
      y += 6;
      pdf.text(`Mensualidades (${selectedPlan.months} meses): ${formatoMoneda((+unidad.preciolista * selectedPlan.mensualidades) / 100 / selectedPlan.months)}`, 15, y);
      y += 8;
      ...
      for (let i = 1; i <= selectedPlan.months; i++) {
        ...
        pdf.text(`${fecha.toLocaleString('default', { month: 'long', year: 'numeric' })}: ${formatoMoneda((+unidad.preciolista * selectedPlan.mensualidades) / 100 / selectedPlan.months)}`, 20, y);
        y += 6;
      }
    } else {
